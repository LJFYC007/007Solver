...
Scenario file -> game and ranges -> concrete decision tree
              -> CPU ESCFR training -> strategy snapshot -> exploitability evaluation
...
`Chips` stores integer tenths of a chip. The scenario has no big-blind or currency conversion. Pot and stack arithmetic use `Chips`; strategy probabilities, regrets and EVs use `float`. EVs are expressed in external chip units. Fractional sizing rounds to the nearest tenth, with exact halves rounded up.
...
`CalculateTerminalSettlement()` determines the winner or tie and the contributions made since a specified start node. `NetPayoffFromStart()` awards the pot share and subtracts those later contributions.
